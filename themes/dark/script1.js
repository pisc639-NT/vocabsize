...
fetch(`https://raw.githubusercontent.com/pisc639-NT/vocabsize/refs/heads/main/themes/dark/style.css?cachebust=${Math.random()}`).then(x=>x.text()).then(x=>document.head.insertAdjacentHTML('beforeend', `<style>${x}</style>">`));